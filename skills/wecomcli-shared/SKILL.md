...
description: wecom-cli 业务技能的公共前置检查、获取机器人及授权真人身份，以及通用输出约束。
---

# wecom-cli 公共前置检查

本技能提供所有 `wecomcli-*` 业务技能共用的 CLI 安装、版本与授权检查，以及通用输出约束。
首次执行任意 `wecom-cli` 命令前，先完成本技能的CLI检查；检查通过后，再执行。检查过一次后，后续执行任意 `wecom-cli` 命令都不再重复检查。
