...
schema_field_labels! {
    ...
    ServiceInfoAlias            => "ServiceInfo.alias",
    ServiceSchemaId             => "ServiceSchema.id",
    ...
    ServiceSchemaRemoteDoc      => "ServiceSchema.remote_doc",
    ...
    MethodSchemaSkills          => "MethodSchema.skills",
    MethodSchemaHidden          => "MethodSchema.hidden",
    MethodSchemaRemoteDoc       => "MethodSchema.remote_doc",
...
pub struct ServiceInfo {
    ...
    /// Alternative names/aliases for this service (e.g. `["contact", "c"]`).
    #[serde_as(as = "EmitDefaultOnError<ServiceInfoAlias>")]
...
pub struct ServiceSchema {
    /// Backend-declared stable identifier for this service, used as the `id`
    /// in remote doc requests when `remote_doc` is in effect.
    #[serde_as(as = "EmitDefaultOnError<ServiceSchemaId>")]
    #[serde(default)]
    ...
    /// When `true`, documentation flags (`--doc` / `--help` / `--schema`)
    /// for this service are generated by the remote doc endpoint instead of
    /// being rendered locally. Overridable per resource / method.
    #[serde_as(as = "EmitDefaultOnError<ServiceSchemaRemoteDoc>")]
    #[serde(default)]
    pub remote_doc: Option<bool>,
...
pub struct ServiceResource {
    /// Backend-declared stable identifier for this resource, used as the
    /// `id` in remote doc requests when `remote_doc` is in effect.
    #[serde_as(as = "EmitDefaultOnError<ServiceResourceId>")]
    #[serde(default)]
    pub id: Option<String>,
    ...
    /// When `true`, documentation flags (`--doc` / `--help` / `--schema`)
    /// for this resource subtree are generated by the remote doc endpoint
    /// instead of being rendered locally. Overrides the service-level value.
    #[serde_as(as = "EmitDefaultOnError<ServiceResourceRemoteDoc>")]
    #[serde(default)]
    pub remote_doc: Option<bool>,
}
...
pub struct MethodSchema {
    /// Backend-declared stable identifier for this method, used as the `id`
    /// in remote doc requests when `remote_doc` is in effect.
    #[serde_as(as = "EmitDefaultOnError<MethodSchemaId>")]
    #[serde(default)]
    pub id: Option<String>,
    ...
    /// List of skills provided by the backend for this method,
    /// used for user guidance when a method call fails.
    /// Overrides [`ServiceSchema::skills`] when non-empty.
    #[serde_as(as = "EmitDefaultOnError<MethodSchemaSkills>")]
    #[serde(default)]
    pub skills: Vec<String>,
    ...
    /// When `true`, documentation flags (`--doc` / `--help` / `--schema`)
    /// for this method are generated by the remote doc endpoint instead of
    /// being rendered locally. Overrides the service / resource-level value.
    #[serde_as(as = "EmitDefaultOnError<MethodSchemaRemoteDoc>")]
    #[serde(default)]
    pub remote_doc: Option<bool>,
...
/// Find a service in `items` by name: an exact `name` match wins over an
/// `alias` match; returns `None` when neither hits.
pub(crate) fn find_service_by_name<'a>(
    items: &'a [ServiceInfo],
    name: &str,
) -> Option<&'a ServiceInfo> {
    items.iter().find(|info| info.name == name).or_else(|| {
        items
            .iter()
            .find(|info| info.name == name || info.alias.iter().any(|a| a == name))
    })
}
...
mod tests {
    ...
    //! - 嵌套资源树 → 按层级正确解析
    //! - alias 解析：name 与 alias 冲突 → 精确 name 优先；均未命中 → None
    //!
    //! ### 上下游交互
    ...
    /// 构造仅含 name/alias 的 ServiceInfo 测试夹具
    fn service_info(name: &str, alias: &[&str]) -> ServiceInfo {
        ServiceInfo {
            name: name.to_string(),
    ...
    /// P0：[find_service_by_name] 按精确 name 命中服务
    /// 条件：items 含 name="hr" 的服务，以 "hr" 查询
    /// 断言：返回 Some 且 name == "hr"
    #[test]
    fn find_service_by_name_exact_match() {
        let items = vec![service_info("hr", &[]), service_info("doc", &[])];
        assert_eq!(
            find_service_by_name(&items, "hr").map(|i| i.name.as_str()),
            Some("hr")
        );
    }

    /// P0：[find_service_by_name] 按 alias 命中服务并解析出规范 name
    /// 条件：items 含 name="hr"、alias=["contact"] 的服务，以 "contact" 查询
    /// 断言：返回 Some 且 name == "hr"
    #[test]
    fn find_service_by_name_alias_match() {
        let items = vec![service_info("hr", &["contact"])];
        assert_eq!(
            find_service_by_name(&items, "contact").map(|i| i.name.as_str()),
            Some("hr")
        );
    }

    /// P1：[find_service_by_name] name 与 alias 冲突时精确 name 优先
    /// 条件：服务 A（name="a"，alias=["b"]）位于服务 B（name="b"）之前，以 "b" 查询
    /// 断言：返回 name == "b" 的服务 B，而非 alias 命中的服务 A
    #[test]
    fn find_service_by_name_prefers_exact_name_over_alias() {
        let items = vec![service_info("a", &["b"]), service_info("b", &[])];
        assert_eq!(
            find_service_by_name(&items, "b").map(|i| i.name.as_str()),
            Some("b")
        );
    }

    /// P1：[find_service_by_name] name 与 alias 均未命中时返回 None
    /// 条件：items 中不存在 name 或 alias 为 "unknown" 的服务
    /// 断言：返回 None
    #[test]
    fn find_service_by_name_miss_returns_none() {
        let items = vec![service_info("hr", &["contact"])];
        assert!(find_service_by_name(&items, "unknown").is_none());
    }
    ...
    /// P1：[MethodSchema] 反序列化 method 级 skills 列表
    /// 条件：JSON 含 skills: ["a", "b"]
    /// 断言：method.skills == ["a", "b"]
    #[test]
    fn deserialize_method_skills() {
        let json = r#"{ "path": "/x", "http_method": "GET", "skills": ["a", "b"] }"#;
        let method: MethodSchema = serde_json::from_str(json).unwrap();
        assert_eq!(method.skills, vec!["a".to_string(), "b".to_string()]);
    }

    /// P1：[MethodSchema] 缺省 skills 时回退为空列表
    /// 条件：JSON 不含 skills
    /// 断言：method.skills 为空
    #[test]
    fn deserialize_method_skills_absent() {
        let json = r#"{ "path": "/x", "http_method": "GET" }"#;
        let method: MethodSchema = serde_json::from_str(json).unwrap();
        assert!(method.skills.is_empty());
    }

    /// P1：[MethodSchema] skills 类型错误时回退为空列表并上报正确 field 标签
    /// 条件：JSON 中 skills 为字符串 "bad"
    /// 断言：method.skills 为空，且收到 field="MethodSchema.skills" 遥测事件
    #[test]
    fn deserialize_method_skills_wrong_type_falls_back() {
        ...
        let method: MethodSchema =
            serde_json::from_str(r#"{ "path": "/x", "http_method": "GET", "skills": "bad" }"#)
                .unwrap();
        drop(_enter);
        ...
        assert_json_diff::assert_json_eq!(
            ...
            serde_json::json!("MethodSchema.skills")
        );
    }
    ...
    /// P0：[MethodSchema] 反序列化 remote_doc 为 true
    /// 条件：JSON 含 remote_doc: true
    /// 断言：method.remote_doc == Some(true)
    #[test]
    fn deserialize_method_remote_doc_true() {
        let json = r#"{ "path": "/x", "http_method": "GET", "remote_doc": true }"#;
        let method: MethodSchema = serde_json::from_str(json).unwrap();
        assert_eq!(method.remote_doc, Some(true));
    }

    /// P1：[MethodSchema] 缺省 remote_doc 时为 None
    /// 条件：JSON 不含 remote_doc
    /// 断言：method.remote_doc == None
    #[test]
    fn deserialize_method_remote_doc_absent() {
        let json = r#"{ "path": "/x", "http_method": "GET" }"#;
        let method: MethodSchema = serde_json::from_str(json).unwrap();
        assert!(method.remote_doc.is_none());
    }

    /// P1：[MethodSchema] remote_doc 类型错误时回退为 None
    /// 条件：JSON 中 remote_doc 为字符串 "yes"
    /// 断言：method.remote_doc == None（DefaultOnError 容错，不 panic）
    #[test]
    fn deserialize_method_remote_doc_wrong_type_falls_back() {
        let json = r#"{ "path": "/x", "http_method": "GET", "remote_doc": "yes" }"#;
        let method: MethodSchema = serde_json::from_str(json).unwrap();
        assert!(method.remote_doc.is_none());
    }

    /// P0：[ServiceSchema] 反序列化 service 级 remote_doc
    /// 条件：JSON 含 remote_doc: true
    /// 断言：schema.remote_doc == Some(true)
    #[test]
    fn deserialize_service_remote_doc_true() {
        let json = r#"{ "base_url": "https://x.com", "remote_doc": true }"#;
        let schema: ServiceSchema = serde_json::from_str(json).unwrap();
        assert_eq!(schema.remote_doc, Some(true));
    }

    /// P0：[ServiceResource] 反序列化 resource 级 remote_doc
    /// 条件：嵌套 resource 含 remote_doc: false
    /// 断言：子 resource 的 remote_doc == Some(false)，根 resource 为 None
    #[test]
    fn deserialize_resource_remote_doc() {
        let json = r#"{
            "base_url": "https://x.com",
            "remote_doc": true,
            "resources": {
                "department": { "remote_doc": false }
            }
        }"#;
        let schema: ServiceSchema = serde_json::from_str(json).unwrap();
        assert!(schema.resource_tree.remote_doc.is_none());
        assert_eq!(
            schema.resource_tree.resources["department"].remote_doc,
            Some(false)
        );
    }

    /// P1：[ServiceSchema] remote_doc 类型错误时回退为 None 并上报正确 field 标签
    /// 条件：JSON 中 remote_doc 为数字 1
    /// 断言：schema.remote_doc == None，且收到 field="ServiceSchema.remote_doc" 遥测事件
    #[test]
    fn deserialize_service_remote_doc_wrong_type_falls_back() {
        let _guard = tracing::subscriber::set_default(
            tracing_subscriber::Registry::default().with(TelemetryLayer::new()),
        );
        let collected: Arc<Mutex<Vec<ClientEvent>>> = Arc::new(Mutex::new(Vec::new()));
        ...
        let schema: ServiceSchema =
            serde_json::from_str(r#"{ "base_url": "https://x.com", "remote_doc": 1 }"#).unwrap();
        drop(_enter);

        assert!(schema.remote_doc.is_none());
        let snaps = std::mem::take(&mut *collected.lock().unwrap());
        assert_eq!(snaps.len(), 1);
        assert_eq!(snaps[0].kind, ctr::KIND);
        assert_json_diff::assert_json_eq!(
            snaps[0].payload[ctr::FIELD_FIELD],
            serde_json::json!("ServiceSchema.remote_doc")
        );
    }

    /// P0：三层 schema 均反序列化 id 字段
    /// 条件：service / resource / method 各声明 id
    /// 断言：三层 id 均为对应值
    #[test]
    fn deserialize_id_fields() {
            ...
            r#"{
                ...
                "resources": {
                    "department": {
                        "id": "res-department",
                        "methods": {
                            "list": { "id": "m-list", "path": "/list", "http_method": "GET" }
                        }
                    }
                }
            }"#,
        ...
        .unwrap();
        assert_eq!(schema.id.as_deref(), Some("svc-hr"));
        let dept = &schema.resource_tree.resources["department"];
        assert_eq!(dept.id.as_deref(), Some("res-department"));
        assert_eq!(dept.methods["list"].id.as_deref(), Some("m-list"));
    }

    /// P1：[MethodSchema] id 缺省时为 None，类型错误时回退 None
    /// 条件：JSON 不含 id；id 为数字 1
    /// 断言：method.id 均为 None
    #[test]
    fn deserialize_method_id_fallbacks() {
        let method: MethodSchema =
            serde_json::from_str(r#"{ "path": "/x", "http_method": "GET" }"#).unwrap();
        assert!(method.id.is_none());
        let method: MethodSchema =
            serde_json::from_str(r#"{ "path": "/x", "http_method": "GET", "id": 1 }"#).unwrap();
        assert!(method.id.is_none());
    }
